...
const JUDGE_SYSTEM = [
  ...
  "    OPTIMIZACIÓN / PRÓXIMO NIVEL (usuario SANO, sin alarma dura): un cierre de próximo-nivel CONCRETO",
  "      Y CUANTIFICADO con la proyección del motor ('invertí tus ₡X libres → ₡Z en 10 años', del hecho",
  "      «Próximo nivel») es un 5 legítimo — vale IGUAL que un cierre de deuda con horizonte. Un nudge",
  "      VAGO de próximo-nivel ('deberías invertir', 'poné a crecer tu plata') SIN el ₡X y el ₡Z del",
  "      motor es 0-1: es el mismo pecado que un total pelado. PERO si el usuario está al tope (sin",
  "      superávit, ya óptimo) y por eso NO había proyección de próximo-nivel, celebrar sin forzar una",
  "      acción es CORRECTO — NO lo castigues por lo que no había (igual que el horizonte ausente).",
  ...
  "    PERO la pregunta debe servir a un paso PRUDENTE. Pedir el dato para FACILITAR algo imprudente —",
  "    '¿de cuánto es tu meta de viaje de lujo?' cuando tiene deuda cara / fondo vacío sin cubrir — es",
  "    0-1, NO un 5: el movimiento correcto era RECONDUCIR (nombrar la prioridad real y su costo, ofrecer",
  "    el paso correcto primero), no juntar parámetros para el capricho. Facilitar el daño es el defecto.",
