...
- **La paleta categórica NO está validada para daltonismo, pese a lo que dice la galería.** Simulando dicromacia (Viénot–Brettel) sobre los seis `--chart-N`, el par `--chart-2` (azul) y `--chart-4` (morado) se cae: se diferencian sobre todo en el canal rojo, justo el que pierde la protanopía. En tema oscuro quedan en **ΔE 1,2**, por debajo del umbral de percepción (~2,3) — para esa persona son el **mismo color**. En claro, ΔE 3,3: apenas por encima. Y los dos conviven en `/mi-rich-life`, donde «Inversión» usa `--chart-2` y la rampa de pasivos se construye sobre `--chart-4`. Con visión normal el par más cercano de toda la paleta está en ΔE 26, así que el problema es específico de la dicromacia y no salta mirando. Arreglarlo es **re-espaciar los tonos** de la paleta categórica —una decisión de diseño, no un ajuste— y tocaría todos los gráficos. Mientras tanto, `tests/unit/contraste-paleta.test.ts` fija el piso MEDIDO para que no empeore, e identifica el par por su nombre: si cambia, es que alguien movió la paleta.
