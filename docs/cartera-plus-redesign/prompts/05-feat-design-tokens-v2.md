...
3. `src/lib/format.ts`: ya tiene `formatMoney`, `formatPercent`, `formatCompact`, `formatAxisCompact`, `formatMonthYear`. Agregá, sin cambiar las existentes:
   - `formatDelta(value, currency, decimals?)` → `+₡2.500` / `−₡2.500` / `₡0` (el cero va sin signo). **Sin `Intl`**: reusa `formatMoney` y antepone el signo, con el menos tipográfico U+2212. La agrupación de miles de este archivo se hace a mano y con PUNTO a propósito — ver la nota 1 del encabezado de `src/lib/format.ts`: `es-CR` en CLDR agrupa con espacio duro en Node y con punto en el WebView de iOS, y el mismo importe salía con dos separadores según dónde se renderizara. Un delta con `Intl` reintroduce ese bug y además convive mal con el `₡2.500` de `formatMoney`.
   - `formatPct1(ratio)` → `12,3 %` (una decimal, coma, espacio duro antes de %).
   - `formatDayMonth(iso)` → `16 ago`; `formatMonthShort(iso)` → `ago 26`.
   - Tests en `tests/unit/format.test.ts` para las nuevas y para `formatMoney` con CRC/USD. Los miles se comparan con **punto** (`₡1.250.000`), no con espacio duro: el espacio duro es lo que emitiría CLDR, y este archivo no usa CLDR. El U+00A0 sí aparece, pero solo como separador tipográfico antes del `%` en `formatPct1`. Incluí un test que afirme que `formatDelta` produce el mismo cuerpo que `formatMoney`, para que nadie lo reimplemente con `Intl`.
