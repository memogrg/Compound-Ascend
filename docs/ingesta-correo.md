...
| **Una sola forma de conectar el correo**: la dirección única. El formulario «correo + código» y los niveles por reenviador/remitente se retiraron del código; la tabla `email_ingest_links` se dropea con `20260905000001` tras el deploy                                                                                                                                                                                                                                                                                           | ✅                      | `chore/ingesta-una-sola-forma`                                                                                 |
| **Conciliador nivel B** (PR #744): «Por revisar» avisa si una propuesta parece un movimiento ya registrado por recibo/manual/chat/importación y ofrece «Sí, es el mismo» (une: el movimiento gana `external_ref`/`card_last4`/`bank_code`, la propuesta queda `merged`); al registrar por otra puerta, la propuesta pendiente que calce se une sola. Regla en `src/lib/ingestion/reconcile.ts`: mismo tipo y moneda, monto al céntimo, fecha ±1 día, comercio parecido                                                              | ✅                      | `reconcile-service.ts`, migración `20260905000002`                                                             |
| **Lote `.eml`** («Reenviar como archivo adjunto») + **Por revisar por fechas y en lote** (PR #745): `decodeMail` abre cada adjunto message/rfc822 como aviso propio; filtro del/al, «Todos», Registrar N / Descartar N (máx. 200)                                                                                                                                                                                                                                                                                                   | ✅                      | `src/lib/ingestion/email/mime.ts`, `por-revisar-card.tsx`, `revision-inbox.tsx`                                |
| **«Buscar avisos ahora»** + cron cada 5 min (PR #742)                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | ✅                      | `run-poll.ts`, `ingest-poll-service.ts`                                                                        |
...
5. **Conciliación automática por referencia (nivel C).** Con `external_ref` ya en `transactions`,
   unir sin preguntar cuando la referencia del banco coincide exacta. Solo por referencia; por
   monto+fecha nunca automático. (El reenvío «como adjunto» quedó cerrado en PR #745.)
