...
export const FULL_CONTEXT_SCOPE: ContextScope = {
  patrimonio: true,
  portfolio: true,
  defense: true,
  flavor: true,
...
export async function buildFinancialContext(
  scope: ContextScope = FULL_CONTEXT_SCOPE,
  ...
  try {
    const { getBaseSummary, getPrimaryCurrency, getDisplayCurrency } =
    ...
    try {
      ...
      if (a.totalPortfolioValue > 0) {
        ...
        ctx.concentracion = {
          ...
          porTipo: Object.values(a.allocation)
            .filter((s) => s.value > 0)
            .map(slice),
  ...
  if (scope.flavor)
    try {
      ...
      try {
        const { getSnapshotHistory: getPortfolioHistory } =
    ...
    try {
      ...
      try {
        ...
        if (draft) {
          ...
          if (typeof draft.perceivedControl === "number")
            ctx.perceivedControl = draft.perceivedControl;
          if (typeof draft.dependentsCount === "number")
            ctx.dependentsCount = draft.dependentsCount;
          if (typeof draft.financialNucleus === "string")
            ctx.financialNucleus = draft.financialNucleus;
          if (typeof draft.hasEmergencyFund === "string")
            ctx.hasEmergencyFund = draft.hasEmergencyFund;
  ...
  if (scope.defense)
    try {
      ...
      const puedeConvertir =
        !!primaryCurrency && !!rates?.[primaryCurrency] && !!rates?.[ctx.currency];
