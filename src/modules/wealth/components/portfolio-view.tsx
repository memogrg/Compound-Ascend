    ...
    <div className="grid">
      ...
      <div className="row" style={{ justifyContent: "space-between", gap: 14, flexWrap: "wrap" }}>
        <div
          className="base-tabs"
          role="tablist"
          aria-label="Secciones de cartera"
          style={{ border: 0 }}
        >
          {TABS.map((t) => (
            <button
              ...
              className={tab === t.id ? "base-tab active" : "base-tab"}
        ...
        <div className="seg" role="group" aria-label="Periodo">
          {PERIODS.map((p) => (
            <button
              key={p}
              type="button"
              className={period === p ? "seg-btn on" : "seg-btn"}
              onClick={() => setPeriod(p)}
            >
              {p}
            </button>
...
function ResumenPanel({
  ...
  const donut: DonutDatum[] = slices.map((a) => ({
    name: a.label,
    value: a.value,
    color: a.color,
  }));
    ...
    <>
      ...
      <section className="stat-strip">
        <div className="card stat">
          ...
          <div
            className={gainPositive ? "delta up" : "delta down"}
            style={{ marginTop: 8, padding: "2px 7px" }}
          >
        ...
        <div className="card stat">
          ...
          <div style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 8 }}>
            {formatPercent(dividendAnalytics.dividendYield)} rentabilidad
          </div>
        </div>
        <div className="card stat">
          <div className="ttl">Ingreso pasivo</div>
          <div className="val" style={{ color: "var(--pos)" }}>
            {formatMoney(dividendAnalytics.monthlyDividends, currency)}
          </div>
          <div style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 8 }}>
            /mes · dividendos
          </div>
        ...
        <div className="card-head">
          <div>
            ...
            <div className="card-sub">
              {analytics.holdingsWithPerformance.length} posición(es) · precios en vivo
            </div>
          </div>
...
function CarteraPanel({
  ...
  const dcaInvestmentIds = new Set(
    summary.investments.filter((i) => i.contribution > 0).map((i) => i.id),
  );
  ...
  return (
    <>
      <div className="card">
        <div className="card-head">
          <div>
            <div className="card-title">Por cuenta</div>
            <div className="card-sub">Agrupado por broker / cuenta</div>
          </div>
          <AddHoldingButton currency={currency} />
        </div>
        {byAccount.length === 0 ? (
          ...
          byAccount.map((g, i) => (
            <div key={g.key} className="acct-row2">
              <div
                className="hold-ic"
                style={{ background: classColors[i % classColors.length], fontSize: 13 }}
              >
              ...
              <div className="hold-val">
                ...
                <div className="d muted">
                  {total > 0 ? formatPercent(g.value / total) : "—"} del total
                </div>
              </div>
            </div>
          ))
        ...
        {byClass.length === 0 ? (
          ...
          byClass.map((g, i) => (
            <div key={g.key} className="sec-row">
              ...
              <span className="nm" style={{ color: "var(--ink-2)" }}>
                {g.key}
              </span>
      ...
      <div className="card">
        <div className="card-head">
          <div>
            <div className="card-title">Todas las posiciones</div>
            <div className="card-sub">{holds.length} posición(es) · ordenadas por valor</div>
          </div>
          <AddHoldingButton currency={currency} />
        </div>
        {holds.length === 0 ? (
          ...
          holds.map((h) => (
            <HoldRow
              ...
              total={total}
              currency={currency}
              dca={h.investmentId ? dcaInvestmentIds.has(h.investmentId) : false}
            />
          ))
        )}
      </div>
      ...
      <div className="card">
        <div className="card-head">
          <div>
            <div className="card-title">DCAs en proceso</div>
            <div className="card-sub">
              Aportes recurrentes activos (informativo, no ejecuta órdenes)
            </div>
          </div>
        </div>
        {dcaPlans.length === 0 ? (
            ...
            <div key={p.id} className="acct-row2">
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div className="hold-sub" style={{ textTransform: "capitalize" }}>
                  {p.horizon ?? "recurrente"} · aporte recurrente
                </div>
              </div>
              <div className="hold-val">
              ...
              >
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <div className="hold-sub">
                    {p.latest.yieldPct != null
                      ? `${formatPercent(p.latest.yieldPct / 100)} rentab.`
                      : (h?.symbol ?? "")}
                  </div>
                </div>
                <div className="hold-val">
    ...
    <>
      <div className="card card-pad">
        ...
        <div className="card-sub" style={{ marginBottom: 8 }}>
          % sobre el inicio del periodo · benchmark no disponible
        </div>
      ...
      <section className="mid-grid">
        <div className="card">
          <div className="card-head">
            <div className="card-title">Mejores posiciones</div>
          </div>
          {best.map((h) => (
            <PerfRow key={h.id} h={h} currency={currency} />
          ))}
        </div>
        <div className="card">
          <div className="card-head">
            <div className="card-title">Peores posiciones</div>
          </div>
          {worst.map((h) => (
            <PerfRow key={h.id} h={h} currency={currency} />
          ))}
...
function HoldRow({
  ...
  const swColor =
    iconGradient(h.assetType, h.symbol).background.match(/var\([^)]+\)/)?.[0] ?? "var(--info)";
  return (
    <div
      className="hold-row"
    ...
    >
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div className="hold-sub">
          {h.quantity.toFixed(h.quantity < 1 ? 6 : 2)} uds · coste{" "}
          {formatMoney(h.averageCost, currency)}
        </div>
      </div>
      <div className="hold-alloc alloc-cell">
        <div className="bar-track">
          <div
            className="bar-fill"
            style={{ width: `${Math.min(100, pct * 100)}%`, background: swColor }}
          />
        </div>
        <div className="l">
          {formatPercent(pct)} · {h.broker?.trim() || "Sin cuenta"}
        </div>
      </div>
      <div className="hold-price price-cell">
        {h.currentPrice != null ? formatMoney(h.currentPrice, currency) : "—"}
      </div>
      <div className="hold-val">
        <div className="v">{formatMoney(h.currentValue, currency)}</div>
        <div className="d" style={{ color: pos ? "var(--pos)" : "var(--neg)" }}>
          {pos ? "+" : ""}
  ...
  return (
    <section className="perf-grid">
      <div className="card card-pad">
        ...
        <span
          className="chip"
          style={{ background: `color-mix(in srgb, ${ring} 16%, transparent)`, color: ring }}
        >
