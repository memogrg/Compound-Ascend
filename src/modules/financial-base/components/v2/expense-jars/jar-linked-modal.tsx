...
export function JarLinkedModal({ jar, onClose }: { jar: Extract<Jar, { kind: "linked" }>; onClose: () => void }) {
  const hasItems = jar.items.length > 0;
  const fixed = jar.fixedFunds ?? [];

  return (
    <Modal title={jar.name} sub={KIND_TITLE[jar.linkedKind] ?? "Elementos vinculados"} onClose={onClose}>
      <div className="modal-body">
        {/* Fondos fijos (solo Ahorro) — siempre disponibles. */}
        {fixed.length > 0 ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: hasItems ? 12 : 0 }}>
            {fixed.map((f) => (
              <div key={f.name} className="list-row" style={{ gridTemplateColumns: "1fr auto" }}>
                ...
                <span className="chip" style={{ fontSize: 10, background: "var(--chip)", color: "var(--muted)" }}>fijo</span>
              </div>
            ))}
          </div>
        ) : null}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {jar.items.map((it) => (
              <div key={it.id} className="list-row" style={{ gridTemplateColumns: "1fr auto" }}>
                ...
                <span className="tnum" style={{ fontSize: 13, fontWeight: 500 }}>{it.amount}</span>
              </div>
            ))}
          </div>
        ) : fixed.length === 0 ? (
