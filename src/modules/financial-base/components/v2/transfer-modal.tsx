...
export function TransferButton({ accounts, currency }: { accounts: Account[]; currency: string }) {
  ...
  return (
    <>
      ...
      {open ? <TransferModal accounts={accounts} currency={currency} onClose={() => setOpen(false)} /> : null}
    </>
  );
}

function TransferModal({ accounts, currency, onClose }: { accounts: Account[]; currency: string; onClose: () => void }) {
      ...
      <Modal title="Transferencia" sub="Mueve dinero entre tus cuentas." onClose={onClose}>
        <div className="modal-body">
          <p className="muted" style={{ fontSize: 13 }}>
            Necesitas al menos <strong>2 cuentas</strong> para transferir. Agrégalas en Configuración.
          </p>
        </div>
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setPending(true);
    setError(null);
    const res = await addTransferAction({ fromAccountId: fromId, toAccountId: toId, amount: amt, currency, occurredOn: date, note: note || undefined });
    setPending(false);
    if (res.ok) {
      ...
      <form onSubmit={onSubmit}>
        <div className="modal-body">
          ...
          <div className="fld">
            ...
            <input className="inp" value={note} onChange={(e) => setNote(e.target.value)} placeholder="Ej. ahorro mensual" />
          </div>
        </div>
        <div className="modal-foot">
          <button type="button" className="btn btn-ghost" onClick={onClose}>Cancelar</button>
          <button type="submit" className="btn btn-primary" disabled={pending}>{pending ? "Registrando…" : "Transferir"}</button>
        </div>
      </form>
    </Modal>
  );
