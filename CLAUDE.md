...
Landing motion: the twelve-month scenario chart loops while in view (0.5 s at January → 6 s run → 3.5 s hold at December → again) and freezes off-screen. **Verify landing-motion behaviour against a production build (`next build && next start -p 3001`), never against `next dev`:** in dev, React StrictMode runs the effect twice, the cleanup disconnects every `ObservadorVigilado` and clears the timers, and the `yaCorrio` guard skips the second run — so in dev no scroll-triggered animation (reveals, the chart, the closing headline) ever fires. Production is unaffected.
